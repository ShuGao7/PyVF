...
for tag in range (1,Count_Fishnet+1):
	fileR=SearchFile(inRasPath,Prep_Path,name_R,format_tif)
	fileE=SearchFile(inRasPath,Prep_Path,name_E,format_tif)

	inRas=arcpy.Raster(fileR)#input raster
	inRas_Ext=arcpy.Raster(fileE)#input raster
	...
	row=inRas.width #The row of DEM

	col=inRas.height # The column of the DEM
	arr_bi = arcpy.RasterToNumPyArray(inRas,nodata_to_value=0)  #Converts a raster to a NumPy array
	arr_a=arr_bi

	# Convert Raster to numpy array

	Xmin=inRas.extent.XMin #Minimum X of Original DEM
	Ymin=inRas.extent.YMin #Minimum Y of Original DEM
	Xmax=inRas.extent.XMax #Maximum X of Original DEM
	Ymax=inRas.extent.YMax #Maximum Y of Original DEM
	xmin=inRas_Ext.extent.XMin #Minimum X of Extened DEM
	ymin=inRas_Ext.extent.YMin #Minimum Y of Extened DEM
	xmax=inRas_Ext.extent.XMax #Maximum X of Extened DEM
	ymax=inRas_Ext.extent.YMax #Maximum Y of Extened DEM
	Allowance_X =int(abs((round(xmin-Xmin))/CellSize))
	Allowance_Y =int(abs((round(ymin-Ymin))/CellSize))

	arcpy.env.extent = arcpy.Extent(xmin,ymin,xmax,ymax)
	LL=arcpy.Point(xmin,ymin)

	Ras=inRas_Ext*1
	arr = arcpy.RasterToNumPyArray(Ras,nodata_to_value=999)
	...
	for x in range(Allowance_Y,row-Allowance_Y): # (x,y) is the location for raster cells
		for y in range(Allowance_X,col-Allowance_X):
			#print x
			A1=[]
			B1=[]
			C1=[]
	...
	myRaster_r = arcpy.NumPyArrayToRaster(N,LL,CellSize,CellSize)
	myRaster_mh = arcpy.NumPyArrayToRaster(H_h,LL,CellSize,CellSize)

	Raster_dh= arcpy.sa.ExtractByMask(myRaster_mh,fileR)
	Raster_r=arcpy.sa.ExtractByMask(myRaster_r,fileR)
